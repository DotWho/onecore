...
;(function() {
  ...
  function extend(to, from, whitelist) {
    ...
    if (whitelistType === 'string') {
      ...
      for (var property in from) {
        if (whitelist) {
          if (
            (whitelistType === 'regexp' && !whitelist.test(property)) ||
  ...
  extend($, {
    ...
    Class: function(o) {
      var special = ['constructor', 'extends', 'abstract', 'static'].concat(
        Object.keys($.classProps)
      )
        ...
        .forEach(function(type) {
          ...
          if (
            listeners[type].filter(function(l) {
              return (
                l.callback === options.callback && l.capture == options.capture
              )
            }).length === 0
          ) {
  ...
  $.setProps = {
    ...
    data: function(name, options) {
      ...
      if (name) {
        return (_new[name] || _new[name] === 0) && Object.keys(_new).length > 0
          ? _new[name]
          : undefined
      } else {
        return _new
    ...
    if ($.type(callback) == 'function') {
      if (on.element && (!(method in $.Element.prototype) || !noOverwrite)) {
        $.Element.prototype[method] = function() {
          return (
            this.subject &&
            $.defined(callback.apply(this.subject, arguments), this.subject)
          )
        }
      }
