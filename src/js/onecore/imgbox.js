...
;(function($, $$) {
    ...
    function oc_setData(el, father) {
        ...
        list.forEach(function(item, i) {
            var images = $(
                `<img class="image" src="${item.url}" style="width:${
                    item.imgWidth
                }px;height:${item.imgHeight}px;left:${(cW - item.imgWidth) /
                    2}px;top:${(cH - item.imgHeight) /
                    2}px"ondragstart="return false;"/>`
            )
    ...
    function oc_event() {
        ...
        $rotate.onclick = function() {
            ...
            if (rotateClass) {
                ...
                $image._.removeClass(rotateClass[0])._.addClass(
                    'rotate' + nextDeg
                )
                $thumbImg._.removeClass(rotateClass[0])._.addClass(
                    'rotate' + nextDeg
                )
                // resizeImage(nextDeg);
        ...
        $thumbnails.onmousemove = function(e) {
            if (thumbX > 0) {
                ...
                var $td = $thumbnails._.find('.thumbDrag'),
                    ...
                    left =
                        parseFloat($td._.style('left')) + (nextDragX - thumbX),
                ...
                if (isVertical) {
                    thumbnailsWidth = [
                        thumbnailsHeight,
                        (thumbnailsHeight = thumbnailsWidth)
                    ][0]
