    ...
    update() {
      ...
      if ($input.length > 0) {
        ...
        for (var i = 0; i < ops.uplist.length; i++) {
          ...
          iup._.before(
            `<label class="btn ils active"><img src="${data.url}"><span data-id="${data.id}">删除</span></label>`
          )
        }
      }
    ...
    _upload(img, num, spanlk) {
      ...
      function uploadComplete(evt) {
        ...
        if (data.result == 1) {
          ...
          $this._.children('input[type="hidden"]').value = JSON.stringify(
            ops.uplist
          )
      ...
      function doProccess(file, el) {
        if (/image\/\w+/.test(file.type)) {
          ...
          reader.onload = function() {
            ...
            var $spanload = $(
              `<label class="btn ils imgload"><img src="${basedata}"><span data-id="${ops.num}">删除</span></label>`
            )
